    ...
    while let Some(data) = rx.recv().await {
        let function = FInput {
            ...
            binary: BinaryType::Path(
                std::env::var("WASM_PATH").expect("missing WASM_PATH from enviornment"),
            ),
        };
...
async fn main() {
    ...
    match server_action.as_str() {
        "catchup" => {
            ...
            let config: CatchupConfig = serde_json::from_str(
                &tokio::fs::read_to_string("catchup.json")
                    .await
                    .expect("No catchup.json found."),
            )
            ...
            let events = plug
                ...
                .await
                .expect("failed to retreive events");
            let last_ledger_processed =
                client::do_catchups_on_events(events.data, config.start).await;
            println!(
                "Finished catchup, processed until ledger {}",
                last_ledger_processed
            );
        }
        "newtable" => {
            let tables: Vec<NewZephyrTable> = serde_json::from_str(
                &tokio::fs::read_to_string("tables.json")
                    .await
                    .expect("No tables.json found."),
            )
            .expect("Invalid tables.json format.");
