...
use stellar_xdr::next::{
    ContractDataDurability, Hash, LedgerEntry, LedgerEntryData, LedgerKey, LedgerKeyContractData,
    LedgerKeyTtl, Limits, ReadXdr, ScAddress, ScSymbol, ScVal, ScVec, VecM, WriteXdr,
};
        ...
        if let LedgerEntryData::ContractData(data) = instance.entry.data {
            if let ScVal::ContractInstance(instance) = data.val {
                if let Some(map) = instance.storage {
                    let supply = map.iter().find(|entry| {
                        entry.key == ScVal::Symbol(ScSymbol("supply".try_into().unwrap()))
                    });
