...
fn with_store(store: Arc<JobsManager>) -> impl Filter<Extract = (Arc<JobsManager>,), Error = std::convert::Infallible> + Clone {
...
async fn main() {
    ...
    let execute = warp::path("execute").and(warp::post()).and(warp::body::json()).and(with_store(manager.clone())).and_then(|body: FunctionRequest, store: Arc<JobsManager>| async move {
        ...
        Ok::<WithStatus<String>, Rejection>(warp::reply::with_status(resp, warp::http::StatusCode::OK))
    });

    let fetch = warp::path!("catchups" / u32).and(warp::get()).and(with_store(manager.clone())).and_then(|id: u32, store: Arc<JobsManager>| async move {
        let status = store.read_job(id).await.unwrap_or("not complete".into());
        Ok::<WithStatus<String>, Rejection>(warp::reply::with_status(status, warp::http::StatusCode::OK))
    });
