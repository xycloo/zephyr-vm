...
use zephyr::{
    db::ledger::LedgerStateRead, host::Host, testutils::database::MercuryDatabase, vm::Vm,
    ZephyrMock,
};
...
mod test {
    ...
    async fn run_entries_filter() {
        let code =
            { read("../target/wasm32-unknown-unknown/release/entries_filter.wasm").unwrap() };
        let execution = ExecutionWrapper::new(&code);
    ...
    async fn run_alert() {
        let code = {
            read("../../zephyr-examples/zephyr-alert/target/wasm32-unknown-unknown/release/zephyr_alert.wasm").unwrap()
        };
        let execution = ExecutionWrapper::new(&code);
